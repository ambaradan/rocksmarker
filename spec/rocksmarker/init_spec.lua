describe("Neovim initialization with rocks.nvim", function()
  ...
  it("should set up rocks.nvim configuration correctly", function()
    ...
    for _, path in ipairs(expected_lua_path) do
      assert.is_true(package.path:find(path, 1, true) ~= nil, string.format("package.path should contain %s", path))
    end
    ...
    for _, cpath in ipairs(expected_c_path) do
      assert.is_true(package.cpath:find(cpath, 1, true) ~= nil, string.format("package.cpath should contain %s", cpath))
    end
